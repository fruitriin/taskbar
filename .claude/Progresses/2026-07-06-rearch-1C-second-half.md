...
### 現在のタスク: rearch-phase1 スライス 1-C 後半（fullWindowList.vue の Composition 化）

計画: `docs/plans/rearch-phase1.md` スライス表 1-C

#### サブタスクチェックリスト

- [x] スクリプト部（333行）を script setup 化。ipcListen 解除関数ペア方式へ移行
- [x] テンプレート・スタイルは不変
- [x] Stage 1（テスト101件・typecheck 0・lint 0 errors/17 warnings・build ✓）/ Stage 2（Critical 0・Warning 1・Suggestion 3）/ 完了処理

##### 2026-07-06 — 1-C 後半完了
**やったこと**: fullWindowList.vue の script setup 化。レビューが全テンプレート参照識別子の網羅・IPC 引数の等価性・TDZ 安全性・リアクティビティ等価を機械的に確認（Critical 0）。指摘反映: undefined ガードに型ガード由来のコメント追加。意図的逸脱2件（console.log 削除・removeAllListeners → ペア解除）はコミットメッセージに明記。
**今の見立て**: Options API 残は index.vue と option.vue のみ（1-D / 1-E）。
**次の自分へ**: 次は 1-B（エントリー統合）か 1-D（option.vue）。1-D では旧 updateOptions リスナーの同時削除（スライス表の備考）を忘れずに。レビュー Suggestion「filteredWindows / visiblePages の純関数切り出し＋テスト」は 1-D 以降で余裕があれば。
**気になっていること**: なし。

#### 日記

##### 2026-07-06 — 1-C 後半開始
**やったこと**: 構造調査。script 333行・style が半分以上を占める。IPC チャンネルが特殊（allProcesses 等）なので useWindows は接続しない（チャンネルが違う）。
**次の自分へ**: removeAllListeners は ipcListen の解除関数ペアに置き換える（他リスナーを巻き込む removeAllListeners は行儀が悪い）。Electron.listen の旧シグネチャ（event 付き）→ ipcListen（payload のみ）へのハンドラ形変更を忘れずに。
**気になっていること**: watch と computed の this 依存の絡み。読んでから判断。
