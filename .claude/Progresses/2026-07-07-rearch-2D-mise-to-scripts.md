...
### 現在のタスク: rearch-phase2 スライス 2-D（mise 廃止 → package.json scripts）

#### チェックリスト
- [x] 全15タスクを package.json scripts へ移植（swiftbuild の echo をクォート化 = グロブ解釈の穴も修正）
- [x] .node-version 作成、.mise.toml 削除。**bun のバージョンピンは意図的に無し**（旧 [tools] bun="latest" も実質ノーピン。必要になったら packageManager フィールドで）。[env] NODE_ENV=development は非移植（process.env.NODE_ENV の直接参照ゼロをレビューが確認）
- [x] settings.json: レビュー Critical 対応 — Bash(bun run *) は install-app（rm -rf 含む）まで通す過剰許可 → 非破壊タスク11件の明示列挙に修正
- [x] ドキュメント更新（レビュー指摘の取りこぼし2件 docs/testing-with-taskbar-helper.md / nativeSrc CLAUDE.md も対応、rearchitecture-plan.md に陳腐化注記）
- [x] 検証: bun run で lint/format/typecheck/test:all/build 全通過
- [x] レビュー（Critical 1 / Warning 3 → 全対応）・完了処理

##### 2026-07-07 — 2-D 完了、Phase 2 完結
**やったこと**: 上記。オーナー実機確認が望ましい残り: `bun run dev` の起動（4画面）。
**次の自分へ**: Phase 2 完結。次は Phase 3（Tauri v2 移行、一気通貫・途中ビルド不通許容）— 大規模なので着手前に計画の鮮度更新＋オーナーとのキックオフ相談を推奨。swiftFilter / autoUpdate の要否確認も残っている。
**気になっていること**: なし。

#### 日記
##### 2026-07-07 — 2-D 開始
**次の自分へ**: [env] NODE_ENV=development は移植しない（electron-vite が自前で設定。挙動差があれば疑うポイントとして記録）。dev タスクの `&`（バックグラウンド cp）は原文どおり移植（意図不明だが挙動同一優先）。
**気になっていること**: settings.json 編集は auto モード分類器に止められる可能性 → 止められたらオーナーに確認。
