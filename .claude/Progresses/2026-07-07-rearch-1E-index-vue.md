...
### 現在のタスク: rearch-phase1 スライス 1-E（index.vue の Composition 化・最難関）

計画: `docs/plans/rearch-phase1.md` スライス表 1-E

#### サブタスクチェックリスト

- [x] script setup 化＋useOptions 結線＋process リスナー自前維持＋$forceUpdate 排除
- [x] 調査漏れの AddFilter.vue も script setup 化 → レンダラーの Options API 全滅
- [x] Stage 1（テスト104件・typecheck 0・lint 0 errors/10 warnings・build ✓）
- [x] Stage 2（Critical 0 / Warning 3 / Suggestion 5 → 記録整合系を全反映: Plan 備考・useWindows 未消費注記・Feedback へバグ候補記録・markRaw/sortArea コメント精緻化）

##### 2026-07-07 — 1-E 完了（Phase 1 残りは 1-F のみ）
**やったこと**: 上記。意図的逸脱: console.log 削除、死にコード削除（grant/granted/openOption/headers/footers data — テンプレート未参照をレビューで確認）、IPC リスナー解除の追加、windowReady 送信位置が displayInfo リスナー登録の後に移動（IPC 非同期のため実害なし・レビュー S7）。
**今の見立て**: Phase 1 は 1-F（UnoCSS）のみ。1-F はオーナー目視確認が完了条件なのでオーナー在席時が良い。
**次の自分へ**: オーナーに実機確認2点を依頼済みか確認: ①タスクバーの D&D と並び（1-E 後の回帰）②アイコンが遅れて届いたとき表示されるか（$forceUpdate 排除の影響）。1-F 着手時は uno.config と Attributify の型定義から。
**気になっていること**: なし。

#### 日記

##### 2026-07-07 — 1-E 開始
**やったこと**: 設計確定。useOptions は結線する（echo ループなし: index.vue は受信のみで deep watch 送信をしない）。useWindows は結線しない（保留ガード非対応）。
**今の見立て**: sort() の orderRule キー（'Headers'/'Footers'）と実引数（'headers'/'footers'）の不一致という**既存バグの疑い**を発見 — 挙動同一原則に従い今回は複製し、バグ分離ルールで記録だけする。
**次の自分へ**: compaction 後は git diff と本チェックリストで再開。旧 updateOptions リスナーの console.log 削除は意図的逸脱としてコミットに明記。
**気になっていること**: updateWindowIcons の $forceUpdate 除去はリアクティビティ理論上は安全だが、実機のアイコン更新確認をオーナー確認項目に入れる。
