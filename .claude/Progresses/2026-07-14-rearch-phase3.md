...
- [x] 3.5 統合 **完了（2026-07-14）**: 署名/notarize 検証済み・Electron/Swift/15依存 削除済み（tag v2-electron-final）・CI を tauri-action 化・ドキュメント刷新・context_task 移植・restart_helper 撤去。残: ペルソナレビュー集約→実機最終確認→v3.0.0 リリース判断
- [x] 完了条件: rearch-phase3.md 末尾のリスト **全達成（2026-07-14。オーナー実機確認「よさそう」）**
...
##### 2026-07-14 — 第2ラウンド確認 OK、第3ラウンド指摘（2クリック問題）対応

**やったこと**: オーナー確認「良さそう」（メニュー座標・設定初期値とも OK）。新規指摘「タスクバーを一度アクティブにしないとウィンドウ切り替えできない（2クリック必要）」に対し、create_taskbar_window に .accept_first_mouse(true) を追加。macOS は非アクティブウィンドウへの first click をアクティブ化に消費してボタンに届けないのが原因で、Tauri は明示オプションが必要（Electron 版とのデフォルト差）。Rust ゲート通過（check/clippy 0・test 37 pass）。

**今の見立て**: フロント変更なしの1行修正。実機で1クリック切り替えになるはず。もし「タスクバー自体がフォーカスを奪うのも嫌（Dock のように完全非アクティブ化したい）」となったら tauri-nspanel（NSPanel + nonactivatingPanel）への切り替えが次の手。
