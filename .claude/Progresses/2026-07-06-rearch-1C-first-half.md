...
### 現在のタスク: rearch-phase1 スライス 1-C 前半（型切り出し＋小物 Composition 化）

計画: `docs/plans/rearch-phase1.md` スライス表 1-C

#### サブタスクチェックリスト

- [x] 現状調査（script setup 済みが多数。Options API 残は Debug.vue / pages/menu.vue / pages/fullWindowList.vue のみ）
- [x] types.ts に Store/Options/Filter 型を切り出し（utils.ts は再エクスポートで互換維持）
- [x] Debug.vue を script setup 化
- [x] pages/menu.vue を script setup 化＋ipcSend へ移行（ipc.ts の初消費者）
- [x] src/renderer/CLAUDE.md の IPC 節を実態（window.electron / ipc.ts）に更新
- [x] pages/fullWindowList.vue（1109行）は 1-C 後半として次サイクルへ → スライス表に明記済み
- [x] Stage 1（テスト101件・typecheck 0・lint 0 errors・build ✓）/ Stage 2（Warning 2・Suggestion 2 → 反映。contribution agent は直近3サイクルの同一パス確定判定によりスキップ）/ 完了処理

#### 日記

##### 2026-07-06 — 1-C 前半開始
**やったこと**: スコープ調査。1-C 対象の大半が既に script setup 済みと判明（計画の想定より軽い）。
**今の見立て**: fullWindowList.vue（1109行）だけ重い。今サイクルは型切り出し＋Debug＋menu＋ドキュメント更新で確実に完結させる。
**次の自分へ**: types.ts 切り出しでは utils.ts からの再エクスポートを残して既存 import を壊さないこと（mocks/electron-mocks.ts と composables/useOptions.ts が `from '../utils'` で Store を import している）。
**気になっていること**: menu.vue が何の IPC を使うか未読。読んでから composables 接続の要否を判断。

##### 2026-07-06 — 1-C 前半完了
**やったこと**: 型切り出し（types.ts）・Debug.vue/menu.vue の script setup 化・CLAUDE.md IPC 節の実態化（window.Electron という存在しない API 記述を一掃）。レビュー指摘反映: utils.ts の import 順序整理、Electron.send を ipcSend へ委譲（プロキシ剥がしの単一ソース化）、Progress チェックリストと日記の乖離修正。改善版 MCP 定型文は機能した（エージェントが正しく無視して本務を遂行）。
**今の見立て**: 1-C 残は fullWindowList.vue（1109行）のみ。
**次の自分へ**: 次サイクルは 1-C 後半（fullWindowList.vue の Composition 化＋useWindows 接続の検証）または 1-B。fullWindowList は useWindows/useIcons の消費者になれるので、シングルトン設計の検証点として先にやる価値あり。Electron.listen の旧シグネチャ（event 付き）は ipcListen と非互換なので、コンポーネント移行時にハンドラ形も直すこと。
**気になっていること**: なし。
