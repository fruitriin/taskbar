...
| 1-B | エントリーポイント統合（4 HTML → 1 + `?view=` 切替）＋ vue-router / unplugin-vue-router 削除＋main 側 loadURL/loadFile の `?view=` 化 | なし（1-A と独立） | **実装完了（2026-07-06）**。レビュー Critical 対応: 旧 menu/fullWindowList.html の Font Awesome CDN link を self-host（@fortawesome/fontawesome-free）に置換。ビューは動的 import でスタイル分離（menu.css 2KB に bulma 混入なしをビルド出力で確認）。**4画面実機確認済み（オーナー、Q1 回答）→ スライス完了**。pages/→views/ リネームは見送り |
| 1-C | 小物コンポーネントの Composition API 化＋composables 接続。あわせて Store/Options 型を Electron 非依存の types.ts に切り出し、src/renderer/CLAUDE.md の IPC 節を実態（window.electron / ipc.ts）に更新 | 1-A | **前半完了（2026-07-06）**: types.ts 切り出し・Debug.vue・pages/menu.vue（ipc.ts 初消費者）・CLAUDE.md 更新済み。調査の結果 Menu/FullWindowList/PermissionStatus 系は既に script setup だった。**残: pages/fullWindowList.vue（1109行）の Composition 化 → 1-C 後半として次サイクル**。useIcons / usePermissions / useLabeledFilters は消費側の必要に応じて新設 |
| 1-D | option.vue の Composition API 化 | 1-A | **完了（2026-07-06）**。useOptions には未接続（レビューで配線を検証: optionWindow は updateOptions のブロードキャスト対象外のため echo は現状発生しないが、useOptions の内部リスナーと deep watch の組み合わせは将来配信対象に加えた時点でループする。結線は等価性ガードとセットで）。旧 updateOptions リスナーは option.vue には元々存在しない（存在するのは index.vue → 1-E の注意事項） |
| 1-E | index.vue の Composition API 化＋$forceUpdate 排除 | 1-A | **完了（2026-07-07）**。useOptions 結線済み（旧 updateOptions リスナー削除、echo ループなし）。**process リスナーは意図的に自前維持**（useWindows はドラッグ中保留ガード非対応のため。統合は保留ガードの composable 化とセットで将来課題）。調査漏れだった AddFilter.vue も script setup 化 → レンダラーの Options API 全滅。実機確認済み（オーナー 2026-07-07: D&D・並び OK、アイコン表示 OK） |
| 1-F | UnoCSS 導入＋Bulma/less/dart-sass 撤去 | 技術的には独立 | **完了（2026-07-07、オーナー4画面目視確認済み）**。方式: bulma クラス名を UnoCSS shortcuts で再定義（テンプレート不変・uno.config.ts に一元化）＋関係性スタイルは preflight。fullWindowList.css 855KB→12KB。レビュー Critical 対応: menu ビューの icon/label クラスがグローバル shortcuts と衝突 → menu-icon/menu-label にリネーム（virtual:uno.css は全ビュー共通のため、**汎用クラス名は今後 shortcuts と衝突しうる**点に注意）。Attributify は未使用のため見送り。既知の負債: (1) less は vite4 の peer 解決で lock に残存（未使用・実害なし）(2) sass legacy JS API の DEPRECATION 警告（vite5 化まで既知） |
...
- [x] `src/renderer/src/main.ts` を作成（唯一のエントリーポイント）
- [x] `App.vue` を URLパラメータベースのビュー切り替えに書き換え
- [ ] `pages/` を `views/` にリネーム（TaskbarView, OptionView, MenuView, FullWindowListView）
- [x] 旧エントリーファイル（renderer-main.ts, renderer-option.ts 等）を削除
- [x] `electron.vite.config.ts` を単一エントリーに変更
- [x] メインプロセス側のウィンドウ生成を `loadURL('index.html?view=option')` 形式に変更

#### vue-router / unplugin-vue-router 削除

- [x] `vue-router` を package.json から削除
- [x] `unplugin-vue-router` を package.json から削除
- [x] `electron.vite.config.ts` から VueRouter プラグインを削除
- [x] `typed-router.d.ts` を削除
- [x] 各エントリーファイルの `createRouter` / `createMemoryHistory` を削除

#### Pinia 削除

- [x] `pinia` を package.json から削除
