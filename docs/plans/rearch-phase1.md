...
| 1-E | index.vue の Composition API 化＋$forceUpdate 排除 | 1-A | **完了（2026-07-07）**。useOptions 結線済み（旧 updateOptions リスナー削除、echo ループなし）。**process リスナーは意図的に自前維持**（useWindows はドラッグ中保留ガード非対応のため。統合は保留ガードの composable 化とセットで将来課題）。調査漏れだった AddFilter.vue も script setup 化 → レンダラーの Options API 全滅。実機のアイコン更新確認（$forceUpdate 排除の影響）はオーナー確認項目 |
