...
| 1-C | 小物コンポーネントの Composition API 化＋composables 接続（Menu / FullWindowList / Debug / PermissionStatus 系）。あわせて Store/Options 型を Electron 非依存の types.ts に切り出し、src/renderer/CLAUDE.md の IPC 節を実態（window.electron / ipc.ts）に更新 | 1-A | useIcons / usePermissions / useLabeledFilters は消費側の必要に応じてここで新設 |
