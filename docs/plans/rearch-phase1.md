...
| 1-E | index.vue の Composition API 化＋$forceUpdate 排除（最難関。drag-session の配線移行含む） | 1-A | 1-C / 1-D 完了後を推奨（パターン確立のため）。**旧 updateOptions/process リスナーの同時削除必須**（dragOrder が参照する appOrder の二重状態化に注意） |
