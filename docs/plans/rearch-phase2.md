...
| 2-A | Oxlint へ移行、ESLint 系5依存の削除 | — | **完了（2026-07-07）**。vp lint ラッパーは v0.2.2 で設定連携未成熟のため不採用（フォールバック規定）→ oxlint 直接実行＋.oxlintrc.json。主要ルールは実機検証で等価、vue テンプレート検査の縮小は受容（Progress アーカイブ 2026-07-07 参照） |
| 2-B | Oxfmt へ移行、prettier 削除 | 2-A | **完了（2026-07-07）**。oxfmt 単体＋`--migrate=prettier` で規約（singleQuote/no-semi）を維持。全 .vue 整形後の4ゲート通過を確認。整形は約36倍高速化（体感: 全体 0.6 秒） |
| 2-C | テストランナーの扱い | — | **決定（Q2 回答）: bun test 継続 → 作業なしでクローズ** |
| 2-D | mise 廃止の実施 | — | **完了（2026-07-07）**。全15タスクを package.json scripts へ移植（bun run <task>）、.node-version 新設、settings.json の許可を非破壊タスクの明示列挙に更新。bun のバージョンピンは意図的に無し。実機確認済み（オーナー 2026-07-07: bun run dev OK） |
