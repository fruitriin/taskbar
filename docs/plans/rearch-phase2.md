...
| 2-D | mise 廃止の実施 | 2-A/2-B と独立 | **決定（Q2 回答）: 廃止。移行先は package.json scripts**（理由: fish 補完・エスケープ・「.mise.toml は発見されない」問題 → package.json なら透過的）。.mise.toml の全タスクを scripts へ移植し、Node バージョンは .node-version へ |
