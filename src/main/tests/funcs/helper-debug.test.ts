...
describe('Helper filterProcesses - Debug', () => {
  ...
  it('デバッグ: フィルタリングなしの場合すべてのウィンドウが残る', () => {
    ...
    console.log('Input windows:', windows.map(w => w.kCGWindowOwnerName))
    console.log('Output windows:', result.map(w => w.kCGWindowOwnerName))
  ...
  it('デバッグ: 複合フィルター（FinderかつWindow名が空）の動作確認', () => {
    ...
    console.log('Input windows:', windows.map(w => `${w.kCGWindowOwnerName}:"${w.kCGWindowName}"`))
    console.log('Output windows:', result.map(w => `${w.kCGWindowOwnerName}:"${w.kCGWindowName}"`))
    ...
    // 空のFinderウィンドウは除外される
    const emptyFinderExists = result.some(w => 
      w.kCGWindowOwnerName === 'Finder' && w.kCGWindowName === ''
    )
  ...
  it('デバッグ: サイズフィルターの動作確認', () => {
    ...
    windows.forEach(w => {
      console.log(`  ${w.kCGWindowOwnerName}: ${w.kCGWindowBounds.Width}x${w.kCGWindowBounds.Height}`)
    })
    console.log('Output windows:', result.map(w => w.kCGWindowOwnerName))
