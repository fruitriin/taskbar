  ...
  describe('filterProcesses', () => {
    ...
    it('Dockはフィルタリングされる', () => {
      ...
      expect(result).not.toContainEqual(
        expect.objectContaining({ kCGWindowOwnerName: 'Dock' })
      )
    })
    ...
    it('taskbar.fmはフィルタリングされる', () => {
      ...
      expect(result).not.toContainEqual(
        expect.objectContaining({ kCGWindowName: 'taskbar.fm' })
      )
    })
    ...
    it('複数のフィルタ条件が正しく適用される', () => {
      ...
      expect(result).toContainEqual(
        expect.objectContaining({ kCGWindowOwnerName: 'Safari' })
      )
      expect(result).toContainEqual(
        expect.objectContaining({ kCGWindowOwnerName: 'Chrome' })
      )
    })
