...
//  taskbar.helper
//
//  🚨 UE TROUBLESHOOTING (Unresponsive Execution / Unexpected Error):
//
//  If this helper process becomes unresponsive or causes UE errors:
//
//  1. **CHECK CODE SIGNING FIRST** (Most Common Issue):
//     - Open Xcode project: `mise run helper`
//     - Go to: taskbar.helper target → Signing & Capabilities tab
//     - Verify: A valid Signing Certificate is selected
//     - Common problem: Wrong certificate selected or expired certificate
//     - Fix: Select "Sign to Run Locally" or a valid development certificate
//     - Rebuild: `mise run swiftbuild`
//
//  2. Verify entitlements are properly configured (taskbar.helper.entitlements)
//  3. Check permissions: Screen Recording, Accessibility
//  4. See CLAUDE.md for detailed troubleshooting steps
//
