...
- **Task Runner**: package.json scripts（`bun run <task>`）— mise は Phase 2 で廃止
...
- `bun run dev` - Start development server (copies helper binary and runs electron-vite dev)
- `bun run build` - Build application for production
- `bun run build:mac` - Create macOS universal binary
- `bun run test` - Run main process tests
...
- `bun run typecheck` - Run TypeScript type checking for both processes
- `bun run typecheck:node` - Type check main process only
- `bun run typecheck:web` - Type check renderer process only

### Native Helper Development

- `bun run helper` - Open Xcode project for Swift helper
- `bun run swiftbuild` - Build Swift helper via command line
...
- **Quick fix**: `bun run helper` → Signing & Capabilities → Select valid certificate → `bun run swiftbuild`

**詳細なトラブルシューティング、UEリスク分析、デバッグ方法は `nativeSrc/taskbar.helper/CLAUDE.md` を参照してください。**

### Installation

- `bun run install-app` - Install built app to /Applications

## Browser-Based UI Testing

### Overview

開発サーバー（`bun run dev`）を起動すると、ブラウザから直接UIにアクセスしてテストできます。Electron APIのモックが自動的に注入され、実際のElectron環境と同じようにUIが動作します。
...
- `src/renderer/` - Vue.js application with multiple entry points:
  - `index.html` / `src/main.ts` - 単一エントリー（`?view=` でビュー切替）
  - `?view=option` - Settings/preferences window
- ビュー切替は `?view=` パラメータと App.vue の動的 import（vue-router は Phase 1 で削除済み）
...
- UI mode: `bun run test:ui` or `bun run test:ui:renderer`
- Coverage: `bun run test:coverage` or `bun run test:coverage:renderer`
